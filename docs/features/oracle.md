...
The module combines `service` to achieve decentralized injection from trusted Oracles such as `Chainlink` to COVENhub Oracle. Each data collection task is called a feed, and its underlying implementation depends on the `service` module. Feed life cycle
...
```bash
coven tx oracle start test-feed \
    --chain-id="coven-test" \
    --from=node0 \
    --fees=0.3coven \
    --broadcast-mode=block
...
```bash
coven tx oracle pause test-feed \
    --chain-id="coven-test" \
    --from=node0 \
    --fees=0.3coven \
    --broadcast-mode=block
...
```bash
coven tx oracle edit test-feed \
    --latest-history=5 \
