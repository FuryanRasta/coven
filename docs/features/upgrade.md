...
```bash
coven tx gov submit-proposal software-upgrade <plan-name> \
  --deposit <deposit> \
...
```bash
coven tx gov deposit <proposal-id> <deposit> --from=<from> --chain-id=<chain-id> --fees=0.3coven -b block -y
```
...
```bash
coven tx gov vote <proposal-id> <option> --from=<from> --chain-id=<chain-id> --fees=0.3coven -b block -y
```
