...
```plain
coven tx mt issue --name=<denom-name> --from=<sender-address>
```
...
```plain
coven tx mt mint <denom-id> --amount=<amount> --data=<data> --from=<sender-address> --recipient=<recipient-address>
```
...
```plain
coven tx mt mint <denom-id> --mt-id=<mt-id> --amount=<amount> --from=<sender-address> --recipient=<recipient-address>
```
...
```plain
coven tx mt edit <denom-id> <mt-id> --data=<data> --from=<sender-address>
```
...
```plain
coven tx mt transfer <sender> <recipient> <denom-id> <mt-id> <amount>
```
...
```plain
coven tx mt burn <denom-id> <mt-id> <amount> --from=<sender-address>
```
...
```plain
coven query mt supply <denom-id> <mt-id>
```
...
```plain
coven query mt balances <owner> <denom-id>
```
...
```plain
coven query mt token <denom-id> <mt-id>
```
...
```plain
coven query mt tokens <denom-id>
```