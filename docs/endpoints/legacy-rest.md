...
The coven versions v1.0.0 (depends on Cosmos-SDK v0.41) and earlier provided REST endpoints to query the state and broadcast transactions. These endpoints are kept in coven v1.0, but they are marked as deprecated, and will be removed after a few versions therefore call these endpoints legacy REST endpoints.
...
All routes are configured under the following fields in `~/.coven/config/app.toml`:
...
- some additional API configuration options are defined in `~/.coven/config/app.toml`, along with comments, please refer to that file directly.

### Legacy REST API Routes

The REST routes present in Covenhub v0.16 and earlier are marked as deprecated via a [HTTP deprecation header](https://tools.ietf.org/id/draft-dalal-deprecation-header-01.html). They are still maintained to keep backwards compatibility, but will be removed after a few versions.
...
<sup>1</sup>: Transactions that don't support Amino serialization are the ones that contain one or more `Msg`s that are not registered with the Amino codec. Currently in the COVENhub, only IBC `Msg`s fall into this case.

### Migrating to New REST Endpoints (from Cosmos-SDK v0.39)

**COVENhub API Endpoints**
...
The same code as integrating with coven-v0.16.3 mainnet. The transaction structure is as follows:
...
```json
{
    ...
    "value": {
        ...
        "signatures": null,
        "memo": "Sent via coven client"
    }
}
```

Where the COVENhub address prefix uses `iaa` instead, which affects the fields:

- value.msg.value.from_adress
- value.msg.value.to_address

Denom uses `ucoven` instead (1coven = 10<sup>6</sup>ucoven), which affects fields:
...
The same code as integrating with coven mainnet, call `POST` `/txs` to send a transaction, as the example below:

```bash
curl -X POST "http://localhost:1317/txs" -H "accept: application/json" -H "Content-Type: application/json" -d "{ \"tx\": {\"msg\":[{\"type\":\"cosmos-sdk/MsgSend\",\"value\":{\"from_address\":\"iaa1rkgdpj6fyyyu7pnhmc3v7gw9uls4mnajvzdwkt\",\"to_address\":\"iaa1q6t5439f0rkvkzl38m0f43e0kpv3mx7x2shlq8\",\"amount\":[{\"denom\":\"ucoven\",\"amount\":\"1000000\"}]}}],\"fee\":{\"amount\":[{\"denom\":\"ucoven\",\"amount\":\"30000\"}],\"gas\":\"200000\"},\"signatures\":[{\"pub_key\":{\"type\":\"tendermint/PubKeySecp256k1\",\"value\":\"AxGagdsRTKni/h1+vCFzTpNltwoiU7SwIR2dg6Jl5a//\"},\"signature\":\"Pu8yiRVO8oB2YDDHyB047dXNArbVImasmKBrm8Kr+6B08y8QQ7YG1eVgHi5OIYYclccCf3Ju/BQ78qsMWMniNQ==\"}],\"memo\":\"Sent via coven client\"}, \"mode\": \"block\"}"
```
