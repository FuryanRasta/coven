...
`coven` is a command line client for the COVENhub network. COVENhub users can use `coven` to send transactions and query the blockchain data.

## Working Directory

The default working directory for the `coven` is `$HOME/.coven`, which is mainly used to save configuration files and data. The COVENhub `key` data is saved in the working directory of `coven`. You can also specify the `coven`  working directory by `--home`.

## Connecting to a Full Node

The `coven` node provides a RPC interface, transactions and query requests are sent to the process listening to it. The default rpc address the `coven` is connected to is `tcp://localhost:26657`, it can also be specified by `--node`.
...
| --home          | string |          | /Users/bianjie/.coven | Directory for config and data        |
...
| --chain-id        | string |          |                       | Chain ID of tendermint node                                                                                    |
| --home            | string |          |                       | Directory for config and data (default "/Users/bianjie/.coven")                                                 |
| --trace           | string |          |                       | Print out full stack trace on errors                                                                           |
